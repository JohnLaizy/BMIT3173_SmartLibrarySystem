...
<flux:header class="relative lg:hidden">
    ...
    {{--
        全系统的纯视觉向下提示。
        只要下方还有内容、且使用者尚未开始向下滑动，箭头就会显示在底部中央。
        它没有 click handler，pointer-events-none 也确保不会覆盖正常操作。
    --}}
    <div
    ...
    >
        <span
            class="flex size-11 items-center justify-center rounded-full border border-blue-500/25 bg-zinc-900/95 text-blue-300 shadow-lg shadow-black/20 backdrop-blur dark:bg-zinc-800/95"
        >
            <svg
    ...
    <script data-navigate-once>
        (() => {
            const updateAppScrollHint = () => {
                ...
                const canScrollDown = window.scrollY + window.innerHeight
                    < document.documentElement.scrollHeight - 8;
                const userHasStartedScrolling = window.scrollY > 48;
                const shouldShowHint = canScrollDown && !userHasStartedScrolling;

                hint.hidden = !shouldShowHint;
                hint.classList.toggle('opacity-100', shouldShowHint);
                hint.classList.toggle('opacity-0', !shouldShowHint);
            };

            window.smartLibraryUpdateAppScrollHint = updateAppScrollHint;
            ...
            if (!window.smartLibraryAppScrollHintInitialised) {
                ...
                document.addEventListener('livewire:navigated', scheduleHintUpdate);
            }
